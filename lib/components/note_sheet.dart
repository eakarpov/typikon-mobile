      ...
      child: Column(
        ...
        children: [
          if (undeclined.isNotEmpty)
            Text(
              // Падеж НЕ НАЗЫВАЕМ. Прежде здесь стояло «остался прежним —
              // именительным», и на устройстве стало видно, что это неправда:
              // помянник читают вслух и пишут родительным, и про «Анны» мы
              // сказали бы «именительный» о слове, которое уже стоит в
              // родительном. Мы знаем ровно одно — что не склоняли сами.
              undeclined.length == 1
                  ? "Этого имени нет в словаре: мы его не склоняли, и оно напечатано так, "
                      "как вы его вписали, — ${undeclined.single}."
                  : "Этих имён нет в словаре: мы их не склоняли, и они напечатаны так, "
                      "как вы их вписали, — ${undeclined.join(", ")}.",
              style: small,
            ),
          if (civil.isNotEmpty)
            Padding(
