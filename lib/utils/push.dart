...
/// есть когда придётся, а иной день и никогда. Толчок приходит в минуту.
///
/// **Толчок пустой, и это не мелочь.** Сервер присылает `data` без текста: он
...
/// толчков не отдаёт вовсе, пока его не откроют руками, и обойти это нельзя.
...
/// спорят, а подстраховывают друг друга — толчок обыкновенно успевает раньше, а
...
Future<String?> _readPushTokenFromFirebase() async {
  ...
  // толчки, спрашивать не о чем. Отказ — обычный ответ, а не поломка.
...
/// Включить толчки: взять ключ доставки и отдать его серверу.
///
...
/// Выключить толчки: отвязать устройство.
...
/// ключ мёртв, и толчки перестали бы приходить молча, без единой ошибки на
...
/// запуска толчки уходили бы в пустоту.
